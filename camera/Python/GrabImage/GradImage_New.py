...
def work_thread(cam=0, pData=0, nDataSize=0):
    ...
    while True:
        ...
        if ret == 0:
            ...
            # 将图像数据转换为NumPy数组
            img_data = np.frombuffer(pData, dtype=np.uint8, count=nDataSize)
            img_data = img_data.reshape((stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nWidth, 3))  # 彩色图像

            # 使用OpenCV显示图像
            cv2.imshow('Image', cv2.cvtColor(img_data, cv2.COLOR_RGB2BGR))
            cv2.waitKey(1)  # 更新显示窗口
