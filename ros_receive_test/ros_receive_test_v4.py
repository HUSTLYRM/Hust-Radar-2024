...
lock = threading.Lock()
global_point_cloud_queue = deque(maxlen=10)  # 创建一个长度为10的队列
...
def callback(msg):
    ...
    with lock:
        global global_point_cloud_queue
        global_point_cloud_queue.append(point_cloud)  # 将新的点云数据添加到队列中
